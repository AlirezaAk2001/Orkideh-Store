...
function App() {
  ...
  const ProtectedRoute = ({ children }) => {
    if (loading) return <LoadingSpinner />;
    // فقط وقتی کاربر لاگین نکرده و ایمیلش تأیید نشده، به /login برو
    if (!currentUser || (currentUser && !currentUser.emailVerified)) {
      // از هدایت بی‌نهایت جلوگیری کن اگر قبلاً توی /login هستیم
      if (location.pathname !== "/login" && location.pathname !== "/verify") {
        return <Navigate to="/login" state={{ from: location }} />;
      }
      return null; // اگر توی /login هستیم، لود نکن
    }
    return children;
  };
