...
// 📌 تعریف لیست ایمیل‌های مجاز برای ادمین در بالای کامپوننت
...
const AuthPage = () => {
  ...
  const sendVerificationEmail = async (email, code) => {
    if (!email) {
      Swal.fire("خطا", "ایمیل گیرنده خالی است.", "error");
      return;
    }
    ...
    const templateParams = {
      ...
      from_name: "پشتیبانی فروشگاه چرخ خیاطی ارکیده",
    };
    ...
    try {
      ...
      console.log("ایمیل تأیید با موفقیت ارسال شد");
    } catch (error) {
      console.error("خطا در ارسال ایمیل:", error.text || error);
      Swal.fire("هشدار", "ارسال ایمیل ناموفق بود، لطفاً بعداً دوباره تلاش کنید.", "warning");
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isLogin) {
        ...
        const redirectPath = redirect || (allowedAdmins.includes(formData.email) ? "/admin" : "/profile");
        ...
        setIsVerifying(true);
        navigate("/verify", { state: { sentCode: code, userEmail: formData.email }, replace: true });
      }
    } catch (error) {
      console.error("ارور Firebase:", error);
  ...
  const handleVerifyCode = async (e) => {
    e.preventDefault();
    if (String(verificationCode).trim() === String(sentCode).trim()) {
      setCodeError("");
      ...
      Swal.fire("موفق", "ایمیل شما تأیید شد. لطفاً وارد شوید", "success");
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (user && !loading) {
        ...
        console.log("وضعیت ایمیل کاربر:", userData?.emailVerified);
        const redirectPath = redirect || (allowedAdmins.includes(user.email) ? "/admin" : "/profile");
        console.log("✅ کاربر لاگین شد، هدایت به:", redirectPath);
        console.log("Current route before navigate:", window.location.pathname);
        if (!isVerifying && window.location.pathname !== "/verify" && window.location.pathname !== "/login") {
          navigate(redirectPath, { replace: true });
        }
      } else if (!isVerifying && !loading && window.location.pathname !== "/verify" && window.location.pathname !== "/login") {
        navigate("/login", { replace: true });
      }
    });

    return () => unsubscribe();
  }, [navigate, redirect, isVerifying, loading]);
              ...
              <div className="w-full max-w-md sm:max-w-lg md:max-w-xl bg-white p-4 sm:p-6 md:p-8 rounded-xl shadow-lg bg-opacity-90">
                ...
                <form onSubmit={handleVerifyCode} className="space-y-2 sm:space-y-3 md:space-y-4" dir="rtl">
                  <p className="text-center text-gray-600 mb-2 sm:mb-4 text-xs sm:text-sm md:text-base">
                    کد تأیید به ایمیل {location.state?.userEmail || formData.email} ارسال شده است. لطفاً آن را بررسی کنید.
                  </p>
                  <input
                    type="text"
